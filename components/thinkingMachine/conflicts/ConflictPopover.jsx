...
export default function ConflictPopover({
  ...
  const copy = getWorkspaceCopy(uiLanguage).conflict;
  const stateLabel = normalizedState === "contradictory" ? copy.contradictory : copy.tension;
      ...
      {isOpen ? (
        ...
        >
          <div className="flex items-start justify-between gap-2">
            <div className="text-[11px] font-semibold text-slate-700">{copy.signal}</div>
            <MetaPill className={meta.className}>{stateLabel}</MetaPill>
          </div>
          ...
          {relatedTitles.length ? (
            <div className="mt-2">
              <div className="text-[10px] font-semibold uppercase tracking-[0.08em] text-slate-400">{copy.related}</div>
          ...
          >
            ...
            {isLoading ? copy.reading : explanation ? copy.refresh : copy.explain}
          </button>
        </div>
      ) : null}
    </div>

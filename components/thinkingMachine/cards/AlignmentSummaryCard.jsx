...
function Section({ items = [], onSelectSignal, resolveLabel }) {
          ...
          return (
            ...
            >
              <div className="text-[12px] leading-relaxed text-slate-600">
                <EmphasizedSummary summary={item.localizedSummary || item.summary} />
              </div>
...
export default function AlignmentSummaryCard({
  ...
  const copy = getWorkspaceCopy(uiLanguage).alignment;
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            {alignedCount > 0 ? (
              <MetaPill className="bg-yellow-100 text-yellow-700">
                {copy.aligned} {alignedCount}
              </MetaPill>
            ) : null}
            {partialAlignmentCount > 0 ? (
              <MetaPill className="bg-emerald-100 text-emerald-700">
                {copy.partial} {partialAlignmentCount}
              </MetaPill>
            ) : null}
            {(counts.unresolved || 0) > 0 ? (
              <MetaPill className="bg-pink-100 text-[#9F1239]">
                {copy.unresolved} {(counts.unresolved || 0)}
              </MetaPill>
            ) : null}
            {divergingCount > 0 ? (
              <MetaPill className="bg-amber-100 text-amber-700">
                {copy.diverging} {divergingCount}
              </MetaPill>
            ) : null}
          </div>

          <Section items={sections.unresolved} onSelectSignal={onSelectSignal} resolveLabel={copy.resolve} />
          <Section items={sections.aligned} onSelectSignal={onSelectSignal} resolveLabel={copy.resolve} />
          <Section items={sections.diverging} onSelectSignal={onSelectSignal} resolveLabel={copy.resolve} />
        </div>
      ) : (

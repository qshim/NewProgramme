export function buildProcessIdeaPrompt({ mode, flow, stageId, modeProfile, historyContext, uiLanguage = "en" }) {
  const guidanceLanguage = uiLanguage === "ko" ? "Korean" : uiLanguage === "ja" ? "Japanese" : "English";
  const systemPrompt = `
...
## STEP 0. Check whether the request fits this workspace

This workspace supports human-centered design reasoning: challenges, observations, evidence, assumptions, constraints,
goals, ideas, options, risks, decisions, meeting notes, stakeholder needs, and questions that can be developed into a reasoning graph.

- Use status "ready" when the input contains enough usable design or decision context to create at least one grounded node.
- Use status "needs_clarification" when the request is relevant but too vague, references missing context or an unavailable asset,
  or asks for an action without supplying the subject or material needed to perform it.
- Use status "unsupported" when the request is outside human-centered design reasoning or asks this workspace to directly perform
  an unsupported output task such as generating production media, executing code, or retrieving unrelated live information.
- For non-ready requests, do not invent nodes. Return user_nodes and cross_connections as empty arrays.
- Explain the mismatch without blame, state what input is needed, and provide 2 or 3 concrete retry prompts.
- Every guidance field, including every suggestedPrompts item, must be written in ${guidanceLanguage}.
- Node labels and node content must always remain in English, regardless of interface language.
...
- For ready requests, respond in English only for graph fields (label, content, suggestion_label, suggestion_content, connection_label).
...
`;
  ...
  "guidance": null | {"title":"string","message":"string","expectedInput":"string","suggestedPrompts":["string"]},

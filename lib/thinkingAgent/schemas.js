...
export const InputRequestStatusSchema = z.enum(["ready", "needs_clarification", "unsupported"]);
export const InputGuidanceSchema = z.object({
  title: z.string(),
  message: z.string(),
...
export const AIAnalysisResultSchema = z.object({
  ...
  guidance: InputGuidanceSchema.nullable(),
  user_nodes: z.array(UserNodeSchema).max(4),
...
export const ConceptDirectionSchema = z.object({
  ...
  promise: z.string(),
  rationale: z.string(),
  principles: z.array(z.string()).min(2).max(4),
  experienceCues: z.array(z.string()).min(2).max(4),
  tradeoff: z.string(),
  evidenceNeeded: z.string(),
});

export const ConceptQuickActionSchema = z.object({
  id: z.string(),
  label: z.string(),
  prompt: z.string(),
});

export const ConceptStudioResultSchema = z.object({
  stage: z.enum(["frame", "explore", "refine", "compare", "converge"]),
  responseSummary: z.string(),
  directions: z.array(ConceptDirectionSchema).min(1).max(4),
  recommendedDirectionIds: z.array(z.string()).max(2),
  question: z.string(),
  quickActions: z.array(ConceptQuickActionSchema).min(2).max(4),
});

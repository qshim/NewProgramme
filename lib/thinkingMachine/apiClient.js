...
export async function developConcept(payload) {
  const response = await axios.post("/api/concept-studio", payload);
  return response.data;
}
...
export async function fetchProjects(options = {}) {
  ...
  if (typeof window !== "undefined") {
    const browserProjects = listBrowserProjects(params);
    try {
      const response = await axios.get("/api/projects", { params });
      let serverProjects = response.data.projects || [];
      if (serverProjects.length === 0 && params.scope === "member") {
        const discoverResponse = await axios.get("/api/projects", {
          params: { ...params, scope: "discover" },
        });
        serverProjects = discoverResponse.data.projects || [];
      }
      const seen = new Set(browserProjects.map((project) => project.id));
      return [
        ...browserProjects,
        ...serverProjects.filter((project) => !seen.has(project.id)),
...
export async function fetchProject(projectId) {
  if (typeof window !== "undefined") {
    const browserProject = getBrowserProject(projectId);
    if (browserProject) return browserProject;
    try {
...
export async function translateWorkspaceContent(payload) {
